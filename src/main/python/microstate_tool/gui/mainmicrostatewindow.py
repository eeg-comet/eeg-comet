...
class MainMicrostateWindow(QMainWindow):
    ...
    def __init__(self, context, parent=None):
        ...
        self.ui.list_eegs = []
        self.ui.save_dir = os.path.join(basepath, 'output')
        if not os.path.exists(self.ui.save_dir):
            os.makedirs(self.ui.save_dir)
        else:
            shutil.rmtree(self.ui.save_dir)
    ...
    def load_preprocessed(self):
        ...
        self.ui.foldername_preprocessed_data = fname
    ...
    def do_clustering(self):
        ...
        if self.ui.step2_smoothgfp_checkbox.isChecked():
            SMOOTHING = int(self.ui.step2_kernel_size_input.text())
        else:
            SMOOTHING = []
        print(SMOOTHING)
