...
class FeatureIO:
    @staticmethod
    def export_features(features, mode, output_folder, filename, export_format='.csv'):
        """
        ...
        output_path = os.path.join(output_folder, f'{filename}_{mode}{export_format}')
        ...
        if mode == 'static':
            if export_format == '.csv':
                ...
                pd.Series(features).to_pickle(output_path)
            elif export_format == '.hdf':
                pd.Series(features).to_hdf(output_path, key='features')
            ...
            if export_format == '.csv':
                ...
                combined_series.to_hdf(output_path, key='features')
            elif export_format == '.json':
                with open(output_path, 'w') as f:
                    json.dump(features, f)
            else:
