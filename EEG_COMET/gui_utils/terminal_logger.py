"""Logger utilities for consistent console/GUI logging in EEG-COMET."""

import os


class EEGCometLogger:
    """Comprehensive logging utility for EEG-COMET with emoji patterns."""

    def __init__(self, log_window=None):
        """Initialize the logger.

        Args:
            log_window (Optional[object]): LogWindow instance for GUI logging.
        """
    ...
    def _get_step_prefix(self, step: str) -> str:
        ...
        Args:
            step (str): Processing step key.

        Returns:
            str: Emoji-prefixed step label.
        """
    ...
    def section_header(self, step: str, title: str = None):
        ...
        Args:
            step (str): Processing step (e.g., 'PREPROCESSING', 'CLUSTERING').
            title (Optional[str]): Custom title. If None, uses the step name.
        """
        if title is None:
    ...
    def toolbox_header(self, main_title: str, subtitle: str):
        ...
        Args:
            main_title (str): Main title (e.g., 'EEG-COMET').
            subtitle (str): Subtitle (e.g., '(EEG Comprehensive Microstate Extraction Toolbox)').
        """
        separator_length = 60
        ...
        main_title_line = (
            "=" * main_padding
            + main_title_with_emoji
            + "=" * (separator_length - main_title_length - main_padding)
        )
        ...
        subtitle_line = (
            "=" * subtitle_padding
            + subtitle
            + "=" * (separator_length - subtitle_length - subtitle_padding)
        )
    ...
    def settings_info(self, step: str, settings_dict: dict):
        ...
        Args:
            step (str): Processing step.
            settings_dict (dict): Settings to log.
        """
    ...
    def progress_update(self, step: str, current: int, total: int, message: str = None):
        ...
        Args:
            ...
            message (Optional[str]): Optional additional message.
        """
        prefix = self._get_step_prefix(step)
    ...
    def file_operation(self, step: str, operation: str, file_path: str):
        ...
        Args:
            step (str): Processing step.
            operation (str): Operation ('Loading', 'Saving', 'Processing').
...
def get_logger(log_window=None) -> EEGCometLogger:
    ...
    Returns:
        EEGCometLogger: The logger instance.
    """
...
def set_log_window(log_window):
    ...
    Args:
        log_window (object): LogWindow instance.
    """
    global _global_logger
    if _global_logger is None:
        _global_logger = EEGCometLogger(log_window)
