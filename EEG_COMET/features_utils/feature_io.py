...
class FeatureIO:
    ...
    @staticmethod
    def export_rof_data(rof_data_dict, feature_type, feature_mode, output_folder, export_format='.csv'):
        """
        ...
        if all_rof_data:
            ...
            # Export the ROF data
            output_path = os.path.join(output_folder, f'{feature_type}_{feature_mode}_ROF_detailed{export_format}')
    ...
    def export_rtf_data(rtf_data_dict, output_folder, export_format='.csv'):
        """
        Export averaged, baseline-corrected RTF matrices for all recordings into a single file.
        ...
        for filename, data in rtf_data_dict.items():
            ...
            if 'post_tms' in transition_averages_bc:
                ...
                alt_keys = [k for k in transition_averages_bc.keys() if k != 'baseline']
                if not alt_keys:
                    continue
